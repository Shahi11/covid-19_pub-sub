...
const task = async (sortKey, sortIndex = -1, tableName, callback) => {
    ...
    await Promise.all(arrayOfPromises1).then(res => {
        // Task 1 Complete
        console.log("============ TASK COMPLETE OF ========================", sortKey);
...
{
    // Fetch live data
    const apiResponse = await fetch("https://covid-193.p.rapidapi.com/statistics", {
        method: 'GET',
        headers: {
    ...
    for(let i = 0; i<apilen; i++) {
        arrayOfPromises.push(new Promise(async (resolve, reject) => {
            await app.locals.db.collection('CovidTallyPubSub').findOne({
                ...
                console.log('Table inserted ', apiResponse.response.length, apiResponse.response[i].country, apiResponse.response[i].cases.total, apiResponse.response[i].tests.total);
                const newDocument = new CovidTallyPubSub(apiResponse.response[i].country, apiResponse.response[i].cases.active, apiResponse.response[i].cases.total, apiResponse.response[i].deaths.total,  apiResponse.response[i].tests.total, apiResponse.response[i].day, apiResponse.response[i].time);
    ...
    if(app.locals.db.collection('CasesSorted').count() != 10 ){
        Promise.all(arrayOfPromises).then(async (res) => {
        ...
        let k = 0;
        app.locals.db.collection("CovidTallyPubSub").find({}).sort(sortedTotalCases).toArray((err,res) =>{
            for(let i =0; i < 10; i++)
            {
                if(res[i].newDocument.country != app.locals.db.collection('CasesSorted').newServiceOne.country)
                {
                    k = 1;
                }
            }
        ...
        app.locals.db.collection("CovidTallyPubSub").find({}).sort(sortedTests).toArray((err,res) =>{
            for(let i =0; i < 10; i++)
            {
                if(res[i].newDocument.country != app.locals.db.collection('tests').newServiceOne[i].country)
                {
                    k = 1;
                }
            }
        ...
        app.locals.db.collection("CovidTallyPubSub").find({}).sort(sortedReverseCases).toArray((err,res) =>{
            for(let i =0; i < 10; i++)
            {
                if(res[i].newDocument.country != app.locals.db.collection('CasesSortedReverse').newServiceOne[i].country)
                {
                    k = 1;
                }
            }
