...
const userinfoB3 = require("../models/userinfoB3");
const topicinfoB3 = require("../models/topicinfoB3");
...
router.post("/login/user", (req, res, next) => {
  ...
  req.app.locals.db.collection("userinfoB3").findOne(
    {
...
router.post("/login/add", (req, res, next) => {
  const newDocument = new userinfoB3(
    req.body.username,
    req.body.email,
    req.body.password
  );
  req.app.locals.db.collection("userinfoB3").insertOne(
    {
      newDocument,
    },
    (err, result) => {
...
router.post("/subscribe/add", (req, res, next) => {
  let key =
    req.body.service8 !== undefined
      ? "service8"
      : req.body.service9 !== undefined
      ? "service9"
      : "service10";
  req.app.locals.db.collection("userinfoB3").updateOne(
    {
      "newDocument.email": req.body.email,
    },
    {
...
router.post("/topictable/add", (req, res, next) => {
  const newDocument = new topicinfoB3(req.body.servicenumber);
  req.app.locals.db.collection("topicinfoB3").insertOne(
    {
      newDocument,
    },
    (err, result) => {
